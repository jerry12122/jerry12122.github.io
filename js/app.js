...
const Loader = {
  ...
  hide: function(sec) {
    if(!CONFIG.loader.start)
      sec = -1
    this.timer = setTimeout(this.vanish, sec||100);
  },
  vanish: function() {
    if(Loader.lock)
      return;
...
const postBeauty = function () {
  ...
  $.each('.reward button', function (element) {
    element.addEventListener('click', function (event) {
      event.preventDefault();
      window.open("https://www.buymeacoffee.com/Jerry12122");
      // var qr = $('#qr')
...
const siteRefresh = function (reload) {
  ...
  vendorCss('mermaid');
  vendorJs('chart');
  if (CONFIG.valine && CONFIG.valine.appId && CONFIG.valine.appKey) {
